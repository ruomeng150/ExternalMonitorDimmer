...
- 可选在快捷键或托盘立即进入屏保时同步锁定 Windows；解锁后恢复原亮度
...
7. 可勾选“快捷键进入屏保时同步锁屏”。启用后，使用自定义快捷键或托盘菜单进入屏保时会同时锁定 Windows；解锁后恢复亮度。此选项不影响自动调光和“跟随 Windows 屏幕保护程序”模式。
8. 点击“应用并开始”。

快捷键和托盘菜单中的“立即进入屏幕保护程序”可以在监控停止时使用。触发后程序会先等待按键或鼠标按钮释放，再将外接显示器调到设定的最低亮度并启动 Windows 黑屏屏保；未启用同步锁屏时，任意键盘或鼠标操作退出屏保后程序会恢复触发前的亮度。启用同步锁屏时，程序会调用 Windows 锁定工作站，锁定期间保持最低亮度，用户解锁后再恢复触发前的亮度。屏保或锁屏启动失败时也会尝试恢复亮度。
