...
7. 可勾选“快捷键进入屏保时同步锁屏”。启用后，使用自定义快捷键或托盘菜单进入屏保时会先锁定 Windows，收到锁定通知后按“锁屏后启动屏保延迟”等待，再启动黑屏屏保；解锁后恢复亮度。此选项不影响自动调光和“跟随 Windows 屏幕保护程序”模式。
8. 可设置“锁屏后启动屏保延迟”（0-30 秒），用于等待锁屏桌面稳定后再启动黑屏屏保。
9. 在启用同步锁屏后，可勾选“锁屏后同步静音”。锁屏后程序会保存并静音 Windows 默认音频输出，解锁后恢复原音量和静音状态。
10. 点击“应用并开始”。

快捷键和托盘菜单中的“立即进入屏幕保护程序”可以在监控停止时使用。触发后程序会先等待按键或鼠标按钮释放，再将外接显示器调到设定的最低亮度。未启用同步锁屏时，程序随后启动黑屏屏保，任意键盘或鼠标操作退出屏保后恢复触发前的亮度。启用同步锁屏时，程序先锁定 Windows，收到锁定通知后按设定延迟启动黑屏屏保，并在解锁后恢复亮度；若同时启用同步静音，解锁时也会恢复原音量和静音状态。屏保、锁屏或音频操作失败时会记录日志，并尝试恢复已保存状态。
